...
export function loadMaterialProfile(id) {
    const profile = MATERIAL_REGISTRY[id];
    if (!profile) {
        throw new Error(`Unknown material: ${id}. Available: ${Object.keys(MATERIAL_REGISTRY).join(', ')}`);
    }
    return profile;
}
export function updateMaterialProfileFromFabrication(materialId, metrics) {
