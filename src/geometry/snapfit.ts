...
export function calculateRequiredLength(deflectionDepth: number, beamThickness: number, maxStrain: number): number {
  return Math.sqrt(
    (3 * deflectionDepth * beamThickness) / (2 * maxStrain),
  ) * 1.10;
}
